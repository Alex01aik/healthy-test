...
class AppointmentService {
  async createOne(req: Request & { body: CreateOneAppointmentArgs }, res: Response) {
  ...
  async validate(userId: string, doctorId: string, date: Date, duration: number) {
    await Promise.all([
  ...
  validateDuration(duration: number): void {
    const isValid = duration >= 20 && duration <= 120 && duration % Number(process.env.MIN_PERIOD) === 0;
    if (!isValid) {
  ...
  async validateDate(date: Date, duration: number, doctorId: string) {
    ...
    if (minutes % Number(process.env.MIN_PERIOD) !== 0) {
      throw { message: `Wrong format, set visit period with ${Number(process.env.MIN_PERIOD)} min range` };
    }
  ...
  async isDoctorAccess(doctorId: string, date: Date, duration: number) {
    ...
    if (weekDayHours?.from && weekDayHours?.to) {
      ...
      this.compareHoursAndMinutesWithDate(weekDayHours.to, this.calculateTillDate(date, duration), 'more');
    } else {
  ...
  compareHoursAndMinutesWithDate(hoursAndMinutes: string, date: Date, condition?: 'less' | 'more') {
